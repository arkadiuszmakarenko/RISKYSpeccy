...
;--------------------------------------------------------
	...
	.area _GSFINAL
	.area _GSINIT
;zxprog.c:357: static unsigned char load_border_phase = 0u;
...
_wcmd_poll:
	...
	add	ix,sp
	push	af
;zxprog.c:361: unsigned char seq = WCMD_SEQ;
	ld	hl, #0x302e
	ld	c, (hl)
;zxprog.c:364: if (seq != WCMD_DONE) {
	ld	a, (#0x302f)
	sub	a, c
	jr	Z, 00107$
;zxprog.c:365: unsigned int dst = (unsigned int)WCMD_DST_LO |
	ld	a, (#0x3030)
	ld	b, #0x00
	ld	hl, #0x3031
	ld	d, (hl)
	...
	ld	-2 (ix), e
	ld	-1 (ix), a
;zxprog.c:367: unsigned int len = (unsigned int)WCMD_LEN_LO |
	ld	a, (#0x3032)
	ld	b, #0x00
	ld	hl, #0x3033
	ld	d, (hl)
	...
	or	a, d
	ld	d, a
;zxprog.c:370: if (len > WCMD_MAX_DATA) {
	xor	a, a
	cp	a, e
	ld	a, #0x02
	sbc	a, d
	jr	NC, 00102$
;zxprog.c:371: len = WCMD_MAX_DATA;
	ld	de, #0x0200
00102$:
;zxprog.c:374: zcopy((unsigned char *)dst,
	pop	hl
	push	hl
	push	bc
	push	de
	...
	call	_zcopy
	pop	bc
;zxprog.c:379: if ((LOADER_FLAGS & LOADER_FLAG_BORDER_ANIM) != 0u) {
	ld	a, (#0x3f11)
	rrca
	jr	NC, 00104$
;zxprog.c:380: zx_border(load_border_cycle[load_border_phase]);
	...
	and	a, #0x07
	out	(_ULA_PORT), a
;zxprog.c:381: load_border_phase = (unsigned char)((load_border_phase + 1u) % 6u);
...
_wcmd_poll_load_border_cycle_65536_48:
	...
	.db #0x06	; 6
	.db #0x04	; 4
;zxprog.c:389: static void rcmd_poll(void)
;	---------------------------------
; Function rcmd_poll
; ---------------------------------
_rcmd_poll:
;zxprog.c:391: unsigned char seq = RCMD_SEQ;
	ld	hl, #0x3f20
	ld	c, (hl)
;zxprog.c:394: if (seq != RCMD_DONE) {
	ld	a, (#0x3f21)
	sub	a, c
	ret	Z
;zxprog.c:395: unsigned int src = (unsigned int)RCMD_SRC_LO |
	ld	a, (#0x3f22)
	ld	e, a
	ld	d, #0x00
	ld	a, (#0x3f23)
...
;	spillPairReg hl
	...
	or	a, h
	ld	d, a
;zxprog.c:397: unsigned char len = RCMD_LEN;
	ld	hl, #0x3f24
	ld	l, (hl)
;	spillPairReg hl
;zxprog.c:399: if (len > RCMD_MAX_LEN) {
	ld	a, #0x40
	sub	a, l
	jr	NC, 00102$
;zxprog.c:400: len = RCMD_MAX_LEN;
	ld	l, #0x40
;	spillPairReg hl
;	spillPairReg hl
00102$:
;zxprog.c:404: (const unsigned char *)src, (unsigned int)len);
	ld	h, #0x00
;	spillPairReg hl
;	spillPairReg hl
;zxprog.c:403: zcopy((unsigned char *)RCMD_BUF_ADDR,
	push	bc
	push	hl
	ld	hl, #0x3f40
	call	_zcopy
	pop	bc
;zxprog.c:406: RCMD_DONE = seq;
	ld	hl, #0x3f21
	ld	(hl), c
;zxprog.c:408: }
	ret
;zxprog.c:425: static void pgcmd_do_out(void) __naked
;	---------------------------------
; Function pgcmd_do_out
; ---------------------------------
_pgcmd_do_out:
;zxprog.c:432: __endasm;
	ld	a, (0x302C) ; read (*((volatile unsigned char *)0x302Cu)) from cart RAM
	ld	bc, #0x7FFD
	out	(c), a ; OUT (C), A with BC=0x7FFD
	ret
;zxprog.c:433: }
;zxprog.c:437: static void pgcmd_poll(void)
;	---------------------------------
; Function pgcmd_poll
; ---------------------------------
_pgcmd_poll:
;zxprog.c:439: unsigned char seq = PGCMD_SEQ;
	ld	hl, #0x302a
	ld	c, (hl)
;zxprog.c:440: if (seq != PGCMD_DONE) {
	ld	a, (#0x302b)
	sub	a, c
	ret	Z
;zxprog.c:441: pgcmd_do_out();
	push	bc
	call	_pgcmd_do_out
	pop	bc
;zxprog.c:442: PGCMD_DONE = seq;
	ld	hl, #0x302b
	ld	(hl), c
;zxprog.c:444: }
	ret
;zxprog.c:447: void nmi_handler_c(void)
;	---------------------------------
; Function nmi_handler_c
; ---------------------------------
_nmi_handler_c::
;zxprog.c:449: wcmd_poll();
	call	_wcmd_poll
;zxprog.c:450: rcmd_poll();
	call	_rcmd_poll
;zxprog.c:451: pgcmd_poll();
	call	_pgcmd_poll
;zxprog.c:452: kbd_poll_publish();
;zxprog.c:453: }
	jp	_kbd_poll_publish
;zxprog.c:458: void main(void)
;	---------------------------------
; Function main
; ---------------------------------
_main::
;zxprog.c:460: zx_startup_clear();
	call	_zx_startup_clear
;zxprog.c:461: zx_border(GREEN); /* startup complete, main poll loop running */
	ld	a, #0x04
	out	(_ULA_PORT), a
00104$:
;zxprog.c:466: wcmd_poll();
	call	_wcmd_poll
;zxprog.c:467: rcmd_poll();
	call	_rcmd_poll
;zxprog.c:468: pgcmd_poll();
	call	_pgcmd_poll
;zxprog.c:469: kbd_poll_publish();
	call	_kbd_poll_publish
;zxprog.c:473: if (LAUNCH_TRIGGER == 0x55u) {
	ld	a, (#0x3f10)
	sub	a, #0x55
	jr	NZ, 00104$
;zxprog.c:477: zx_launcher();
	call	_zx_launcher
;zxprog.c:480: }
	jr	00104$
	.area _CODE
