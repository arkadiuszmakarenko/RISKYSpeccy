...
void RunCartWithRAM (void) {
    static volatile uint8_t ram[0x1000];
    uint16_t address = (uint16_t)GPIOE->INDR;
     ...
     if ((GPIOB->INDR & GPIO_Pin_5) == 0) { // Check for RD active (active low)
        if (address < 0x3000u) {
            ...
            GPIOD->CFGLR = 0x44444444; // Set GPIOD pins 0-7 back to input
        } else if (address <= ZX_ROM_LAST_ADDRESS) {
            GPIOD->CFGLR = 0x33333333; // Set GPIOD pins 0-7 as output
            GPIOD->OUTDR = (GPIOD->OUTDR & ~0x00FFu) | ram[address - 0x3000u];
            while ((GPIOB->INDR & GPIO_Pin_5) == 0) { }
            GPIOD->CFGLR = 0x44444444; // Set GPIOD pins 0-7 back to input
        }
    } else {
        while (((GPIOB->INDR & GPIO_Pin_10) == 0u) && ((GPIOB->INDR & GPIO_Pin_4) != 0u)) { }

        if (((GPIOB->INDR & GPIO_Pin_10) == 0u) && ((GPIOB->INDR & GPIO_Pin_4) == 0u)) { // Check for delayed WR while MREQ remains active
            if ((address >= 0x3000u) && (address <= ZX_ROM_LAST_ADDRESS)) {
                GPIOD->CFGLR = 0x44444444; // Data bus must be input while capturing host writes
                do {
                    ram[address - 0x3000u]  = (uint8_t)(GPIOD->INDR & 0x00FFu);
                } while ((GPIOB->INDR & GPIO_Pin_4) == 0);
            }
        }
    }
