...
static uint8_t IAP_Is_Flash_Range_Valid (uint32_t address, uint32_t length) {
    if (address < DEF_APP_CODE_START_ADDR) {
        return 0;
    }
    if (address >= DEF_APP_CODE_END_ADDR) {
        return 0;
    }
    if (length == 0) {
        return 1;
    }
    return (length <= (DEF_APP_CODE_END_ADDR - address));
}
...
uint8_t IAP_Flash_Read (uint32_t address, uint8_t *buff, uint32_t length) {
    ...
    if (IAP_Is_Flash_Range_Valid (read_addr, read_len)) {
        /* Available Data */
...
uint8_t IAP_Flash_Write (uint32_t address, uint8_t *buff, uint32_t length) {
    ...
    /* Verify Address, No flash operation if the address is out of range */
    if (IAP_Is_Flash_Range_Valid (write_addr, write_len)) {
...
uint8_t IAP_Flash_Erase (uint32_t address, uint32_t length) {
    ...
    /* Verify Address, No flash operation if the address is out of range */
    if (IAP_Is_Flash_Range_Valid (erase_addr, erase_len)) {
        for (i = 0; i < page_cnts; i++) {
            /* Verify Keys, No flash operation if keys are not correct */
            if (Flash_Operation_Key0 != DEF_FLASH_OPERATION_KEY_CODE_0) {
                /* ERR: Risk of code running away */
...
uint8_t IAP_USBH_PreDeal (void) {
    ...
    /*
     * If the drive is already plugged in at boot, USBFS_UIF_DETECT may not fire,
     * so synthesize a connect event once to start enumeration without replug.
     */
    if ((ret == ROOT_DEV_FAILED) && hw_attached &&
        (RootHubDev[usb_port].bStatus != ROOT_DEV_SUCCESS)) {
        DUG_PRINTF ("USB attached (polled), force enum\r\n");
...
void IAP_Main_Deal (void) {
    ...
    if ((pff_disk_status >= 2) && op_flag)  // 2 = mounted
    {
        ...
        /* Found file, start IAP processing */
            ...
            if (pff_file_size > DEF_APP_CODE_MAXLEN) {
                DUG_PRINTF ("IAP too large: %lu > %lu\r\n", (unsigned long)pff_file_size, (unsigned long)DEF_APP_CODE_MAXLEN);
                DUG_PRINTF ("APP region: 0x%08X..0x%08X\r\n", (unsigned int)DEF_APP_CODE_START_ADDR, (unsigned int)(DEF_APP_CODE_END_ADDR - 1));
                blinkLed (100, 100);
                return;
            }
            ...
            while (totalcount > 0) {
                ...
                uint16_t max_chunk_size = 256;  // Smaller chunks are more robust on some USB sticks
                ...
                uint16_t attempt_size = read_size;

                do {
                    pf_res = pf_read (&IAPLoadBuffer[IAP_WriteIn_Length], attempt_size, &bytes_read);
                    ...
                    if (pf_res == FR_DISK_ERR) {
                        DUG_PRINTF ("Disk err, retry %u @%lu, req=%u\r\n", (unsigned int)(retry_count + 1), (unsigned long)total_read, (unsigned int)attempt_size);
                ...
                if (pf_res != FR_OK) {
                    DUG_PRINTF ("Read err: %d @%lu/%lu\r\n", pf_res, (unsigned long)total_read, (unsigned long)pff_file_size);
                    break;
                }
                ...
                if (IAP_WriteIn_Length == DEF_MAX_IAP_BUFFER_LEN || totalcount == 0) {
                    ...
                    if (ret != 0) {
                        DUG_PRINTF ("Flash err @0x%08X (ret=%lu)\r\n", (unsigned int)write_addr, (unsigned long)ret);
                        break;
                    }
            ...
            if (pff_file_size == IAP_WriteIn_Count) {
                ...
                if (pf_res != FR_OK) {
                    ...
                    while (vfy_rem > 0 && vfy_err == 0) {
                        uint16_t chunk = (vfy_rem > DEF_MAX_IAP_BUFFER_LEN) ? DEF_MAX_IAP_BUFFER_LEN : (uint16_t)vfy_rem;
