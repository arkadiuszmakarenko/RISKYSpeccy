...
DRESULT disk_readp (BYTE *buff, DWORD sector, UINT offset, UINT count) {
    ...
    for (int read_attempt = 0; read_attempt < 8; read_attempt++) {
        if (msc_bulk_in_endp == 0 || msc_bulk_out_endp == 0) {
            return RES_NOTRDY;
        }
        ...
        if (msc_send_cbw (cdb, 10, 512, 0x80) != RES_OK) {
            DUG_PRINTF ("disk_readp CBW fail: sec=%lu try=%d\r\n", (unsigned long)sector, read_attempt);
            goto read_retry;
        }
        ...
        for (uint8_t packet = 0; packet < packets_needed; packet++) {
            ...
            do {
                ...
                // Keep retrying on NAK; only clear endpoint halt on actual STALL.
                if (usb_res == (ERR_USB_TRANSFER | USB_PID_STALL)) {
                    uint8_t ep0 = 64;
                    USBFSH_ClearEndpStall (ep0, (uint8_t)(0x80 | msc_bulk_in_endp));
                    msc_bulk_in_tog = 0;
                    for (volatile int i = 0; i < 3000; i++);
            ...
            } while (nak_retries < 80 && usb_res != ERR_SUCCESS);

            if (usb_res != ERR_SUCCESS || packet_len != 64) {
                DUG_PRINTF ("disk_readp data fail: sec=%lu pkt=%u try=%d usb=%u len=%u\r\n",
                            (unsigned long)sector,
                            (unsigned int)packet,
                            read_attempt,
                            (unsigned int)usb_res,
        ...
        if (msc_receive_csw() != RES_OK) {
            DUG_PRINTF ("disk_readp CSW fail: sec=%lu try=%d\r\n", (unsigned long)sector, read_attempt);
            goto read_retry;
        }
...
read_retry:
        ...
        // If repeated BOT resets cannot recover, re-run full disk initialization.
        if (read_attempt >= 3) {
            disk_reset_state();
    ...
    DUG_PRINTF ("disk_readp fail: sec=%lu off=%u cnt=%u sense=%02X/%02X/%02X\r\n",
                (unsigned long)sector,
                (unsigned int)offset,
                (unsigned int)count,
                (unsigned int)(sense_data[2] & 0x0F),
                (unsigned int)sense_data[12],
                (unsigned int)sense_data[13]);

    DUG_PRINTF ("disk geom: last_lba=%lu block=%lu\r\n", (unsigned long)last_lba, (unsigned long)block_size);
